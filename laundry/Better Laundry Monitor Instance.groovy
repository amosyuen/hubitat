...
def thresholdPage() {
	dynamicPage(name: "thresholdPage") {
		...
		section("<b>Time Thresholds</b>", hidden: false) {
			input "activeDurationSeconds", "number", title: "Number of seconds the reading is above start threshold before it is considered started.", required: false, defaultValue: 300
...
def toRunning() {
	...
	if (messageStart != null && messageStart != "") {
		notify(messageStart, repeatNotificationCount)
	}
}
...
def toFinished() {
	...
	if (messageEnd != null || messageEnd != "") {
		notify(messageEnd, repeatNotificationCount)
	}
}
...
def notify(msg, repeatCount) {
	if (!isState(STATE_FINISHED())) {
		return
	}
	notifyRepeat([msg: msg, repeatCount: repeatCount])
}
...
def notifyRepeat(data) {
	...
	if (data.repeatCount >= 1) {
		runIn(repeatNotificationDelay, notifyRepeat, [data: [msg: data.msg, repeatCount: data.repeatCount - 1]])
	}
}
