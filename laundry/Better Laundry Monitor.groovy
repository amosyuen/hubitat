...
def notificationPage() {
	dynamicPage(name: "notificationPage") {
		section (title: "<b>Notification Methods</b>") {
			input "pushNotificationDevices", "capability.notification", title: "Push Notification Devices", multiple: true, required: false
			input "speechNotificationDevices", "capability.speechSynthesis", title:"Speech Synthesis Devices", multiple: true, required: false
			input "speechEchoAnnouncement", "bool", title:"Use 'Echo Speaks' Play Announcement All (only need one echo device in above list)", defaultValue: false, required: false
			input "musicPlayerDevices", "capability.musicPlayer", title:"Speech Synthesis Music Players", multiple: true, required: false
			input "blockSpeechNotificationSwitch", "capability.switch", title: "Switch to block speech synthesis if ON", multiple: false, required: false
			input "repeatNotificationCount", "number", title:"Number of extra times to notify while contact sensor is closed", defaultValue: 0, required: false
			input "repeatNotificationDelay", "number", title:"Delay in seconds between the each announcements", defaultValue: 30, required: false
		}
	}
}
